...
/**
 * JottoGameData stores the data necessary for a single Jotto game. Note
 * that this class works in concert with the JottoGameStateManager, so
 * all instance variables have default (package-level) access.
 * 
 * @author Richard McKenna
 * @co-author Thomas Marion
 */
public class JottoGameData
{
    ...
    /**
     * Accessor method for testing to see if the secred word
     * has been found this game or not.
     * 
     * @return true if the secret word has been found this
     * game, false otherwise.
     */
    public boolean isWordFound() { return wordFound; }
    
    /**
     * Accessor method for getting the number of guesses the
     * player made this game.
     * 
     * @return The number of guesses the player made this game.
     */
    public int getNumGuesses()  { return guesses.size(); }
    
    /**
     * Accessor method for testing to see if the user has made
     * at least one guess.
     * 
     * @return true if the user has made at least one guess,
     * false otherwise.
     */
    public boolean hasGuessBeenMade() { return !guesses.isEmpty(); }
     ...
     */
    public Iterator<String> guessesIterator() { return guesses.iterator(); }
    ...
    /**
     * Calculates the number of letters in the guess argument
     * that are in the secret word. Note that if guess has
     * repeating characters that are in the secret word, both
     * are counted. So, if the secret word is RHYME, and the
     * guess is MUMMY, this method would return 4, meaning one
     * for each M, and one for Y.
     * 
     * @param guess The word to test against the secret word.
     * 
     * @return The number of letters in guess that are in this
     * game's secret word.
     */
    public int calcLettersInGuess(String guess)
    {
        
        int lettersInGuess = 0;
        String theSecretWord = getSecretWord();
        
        //nested loop to traverse and compare guess with secret word.
        for(int i=0;i<guess.length();i++) {
            for(int j=0;j<theSecretWord.length();j++) {
                
                //if letter in guess = a letter in word
                if(guess.charAt(i)==theSecretWord.charAt(j)) {
                    
                    //return value ++, skips to next letter in guess to avoid
                    //duplicate, and returns to beginning of secret word.
                    lettersInGuess++;
                    break;
                }
            }
        }
        return lettersInGuess;
    }
    
    /**
     * This method tests the guessWord argument to see if it is the secret
     * word. If it is, the game is over, the player has won, and so the
     * game data is updated accordingly.
     * 
    ...
    public void guess(String guessWord)
    {
        ...
        if(guessWord.equals(getSecretWord())) {
            wordFound=true;
    ...
    /**
     * Called when a player quits a game before guessing the
     * secret word, not that it simply calculates the end time.
     * Also note that it does not change the wordFound variable,
     * since that is used to keep track of wins and losses.
     */
    public void giveUp()
    {
